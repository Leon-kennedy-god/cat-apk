...
- **最新版直链**：[喵喵助手 APK](https://github.com/Leon-kennedy-god/cat-apk/releases/latest/download/MeowMeowAssistant.apk)（点击即可下载安装，始终指向最新版）
...
- **两种处理模式**：标点触发（推荐）/ 实时处理
- **高级设置（v1.1）**：仅处理聚焦输入框（防止误识别提示词/占位文字）+ 流式输入稳定防抖（语音输入时等待输入成型再改写，默认 800ms）；v1.1.1 起自动拦截官方提示词（清空/发送后出现的短文本）
